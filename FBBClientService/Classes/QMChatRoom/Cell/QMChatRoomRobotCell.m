...
- (void)createLabel: (NSString *)text {
    ...
    tLabel.checkColor = [UIColor colorWithRed:68/255.0f green:126/255.0f blue:226/255.0f alpha:1];
...
- (void)createImage: (NSString *)imageUrl {
    ...
    if (temArray.count >= 2) {
        ...
        if (loc != NSNotFound) {
            ...
            [imageView sd_setImageWithURL:[NSURL URLWithString:src] placeholderImage:[UIImage imageNamed:@"icon" inBundle:Bundle compatibleWithTraitCollection:nil]];
