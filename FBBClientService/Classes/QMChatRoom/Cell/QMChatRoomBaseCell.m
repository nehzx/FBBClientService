...
- (void)setUI {
    ...
    self.chatBackgroudImage.backgroundColor = [UIColor colorWithRed:246/255.0 green:246/255.0 blue:246/255.0 alpha:1];
...
//    self.timeLabel.text = [QMDateManager showChatTime:message.createdTime];
    ...
    self.timeLabel.text = [QMWeiXinDateManager getTimeStringAutoShort2:confromTimesp mustIncludeTime:true];
    // 重写
    if ([message.fromType isEqualToString:@"0"]) {
        //发送
        if ([NSURL URLWithString:avater]) {
            [self.iconImage sd_setImageWithURL:[NSURL URLWithString:avater] placeholderImage:[UIImage imageNamed:@"qm_default_user" inBundle:Bundle compatibleWithTraitCollection:nil]];
        }else {
            self.iconImage.image = [UIImage imageNamed:@"qm_default_user" inBundle:Bundle compatibleWithTraitCollection:nil];
        }
        self.iconImage.frame = CGRectMake(kScreenWidth-20, CGRectGetMaxY(self.timeLabel.frame)+10, 0, 0);
        
        UIImage *image = [UIImage imageNamed:@"SenderTextNodeBkg" inBundle:Bundle compatibleWithTraitCollection:nil];
        ...
        if ([message.userType isEqualToString:@"system"]) {
            [self.iconImage sd_setImageWithURL:[NSURL URLWithString:[QMConnect sdkSystemMessageIcon]] placeholderImage:[UIImage imageNamed:@"qm_default_robot" inBundle:Bundle compatibleWithTraitCollection:nil]];
        }else {
            if ([NSURL URLWithString:message.agentIcon]) {
                [self.iconImage sd_setImageWithURL:[NSURL URLWithString:message.agentIcon] placeholderImage:[UIImage imageNamed:@"qm_default_agent" inBundle:Bundle compatibleWithTraitCollection:nil]];
            }else {
                if ([message.isRobot isEqualToString:@"1"]) {
                    self.iconImage.image = [UIImage imageNamed:@"qm_default_robot" inBundle:Bundle compatibleWithTraitCollection:nil];
                }else {
                    self.iconImage.image = [UIImage imageNamed:@"qm_default_agent" inBundle:Bundle compatibleWithTraitCollection:nil];
                }
            }
        }
        
        self.iconImage.frame = CGRectMake(20, CGRectGetMaxY(self.timeLabel.frame)+10, 0, 0);
        
        UIImage *image = [UIImage imageNamed:@"ReceiverTextNodeBkg" inBundle:Bundle compatibleWithTraitCollection:nil];
        self.chatBackgroudImage.image = image;
        self.chatBackgroudImage.image = [self.chatBackgroudImage.image stretchableImageWithLeftCapWidth:20 topCapHeight:20];
    ...
    if ([message.fromType isEqualToString:@"0"]) {
        if ([message.status isEqualToString:@"0"]) {
            ...
            self.sendStatus.image = [UIImage imageNamed:@"icon_send_failed" inBundle:Bundle compatibleWithTraitCollection:nil];
            [self removeSendingAnimation];
        } else if ([message.status isEqualToString:@"2"]) {
            self.sendStatus.hidden = NO;
            self.sendStatus.image = [UIImage imageNamed:@"icon_sending" inBundle:Bundle compatibleWithTraitCollection:nil];
