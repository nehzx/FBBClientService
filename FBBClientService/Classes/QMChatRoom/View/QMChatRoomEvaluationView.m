...
- (void)createUI {
    ...
    for (int i = 0; i < self.evaluation.evaluats.count; i++) {
        ...
        [button setImage:[UIImage imageNamed:@"evaluat_icon_nor" inBundle:Bundle compatibleWithTraitCollection:nil] forState:UIControlStateNormal];
        [button setImage:[UIImage imageNamed:@"evaluat_icon_sel" inBundle:Bundle compatibleWithTraitCollection:nil] forState:UIControlStateSelected];
...
//    self.radioView.frame = CGRectMake(25, self.optionHeight + 25, self.backView.frame.size.width - 50, [self calcHeight:self.evaluation.evaluats[number].reason]);
...
//
//    CGFloat maxWidth = self.backView.frame.size.width - 50;
