...
- (void)viewWillAppear:(BOOL)animated {
    ...
    self.navigationController.navigationBar.tintColor = [UIColor colorWithRed:42/255.0 green:42/255.0 blue:42/255.0 alpha:1];
...
- (void)createUI {
    ...
    [self.manualButotn setTitleColor:[UIColor colorWithRed:68/255.0 green:126/255.0 blue:226/255.0 alpha:1] forState:UIControlStateNormal];
...
- (void)getNewReload: (NSNotification *)sender {
    ...
    if ([QMConnect customerAccessAfterMessage]) {//访客先说话才接入
        if (_isSpeak) {//已经说话
            if (!_isRobot && !alreadEvaluate) {//不是机器人 并且 没有评价过
                [QMConnect customerServiceIsSpeek:^{ // 坐席已经说过话
                    dispatch_async(dispatch_get_main_queue(), ^{
  ...
  if (![message.messageType isEqualToString:@"newCardInfo"] && ![message.messageType isEqualToString:@"cardInfo"] && ![message.messageType isEqualToString:@"withdrawMessage"] && ![message.messageType isEqualToString:@"msgTask"] && ![message.messageType isEqualToString:@"image"] && ![message.messageType isEqualToString:@"voice"]) {
    ...
    if ([message.messageType isEqualToString:@"text"]) {
        if ([message.isRobot isEqualToString:@"1"]) {
            ...
            CGFloat titleHeight = [QMAlert calculateRowHeight:message.robotFlowTip fontSize:15.0f Width:240];
            if (titleHeight < 20) {
                titleHeight = 20;
            }
...
- (void)changeButtonStatus:(BOOL)down {
    if (down == YES) {
        ...
        [self.chatInputView.RecordBtn setTitleColor:[UIColor colorWithRed:251/255.0f green:206/255.0f blue:0.0 alpha:1.0] forState:UIControlStateNormal];
        self.chatInputView.RecordBtn.layer.borderColor = [[UIColor colorWithRed:251/255.0f green:206/255.0f blue:0.0 alpha:1.0] CGColor];
        [self.chatInputView.RecordBtn setTintColor:[UIColor colorWithRed:251/255.0f green:206/255.0f blue:0.0 alpha:1.0]];
    }else {
...
- (void)takePicBtnAction {
    ...
    imagePickerVc.naviTitleColor = [UIColor colorWithRed:42/255.0 green:42/255.0 blue:42/255.0 alpha:1];
    imagePickerVc.barItemTextColor = [UIColor colorWithRed:42/255.0 green:42/255.0 blue:42/255.0 alpha:1];
    imagePickerVc.iconThemeColor = [UIColor colorWithRed:251/255.0 green:206/255.0 blue:0.0 alpha:1];
    imagePickerVc.oKButtonTitleColorNormal = [UIColor colorWithRed:251/255.0 green:206/255.0 blue:0.0 alpha:1];
    imagePickerVc.oKButtonTitleColorDisabled = [UIColor colorWithRed:251/255.0 green:206/255.0 blue:0.0 alpha:1];
    ...
    if (_isRobot) {
        ...
        if (self.evaluation.CSRAging && timestamp.length > 0 && self.evaluation.timeout.length > 0) {
            ...
            [QMConnect sdkCheckImCsrTimeoutParams:params success:^{
                ...
                [QMAlert showMessageAtCenter:@"评价已超时"];
            }];
...
- (void)sendBtnAction:(UIButton *)button  {
    if (self.isAleardyChat) {
        ...
        [QMAlert showMessageAtCenter:@"正在建立会话中,请稍后"];
    }
}
...
- (void)insertEmoji: (NSString *)code {
    ...
    if ([plistDict objectForKey:code] != nil) {
        ...
        emojiTextAttemt.image = [UIImage imageNamed:[NSString stringWithFormat:@"%@/%@", bundlePath, [plistDict objectForKey:code]] inBundle:Bundle compatibleWithTraitCollection:nil];
...
- (void)logoutAction {
    ...
    if (isRemark && !_isRobot && isShowEvaluateBtn) {
        NSLog(@"应该显示满意度评价弹框");
...
- (void)currentAgentInfoWithAgent:(QMAgent * _Nonnull)agent {
    ...
    _titleView.nameLabel.text = [NSString stringWithFormat:@"%@",agent.name];
}
...
- (void)customVIP {
    UIAlertController * alertController = [UIAlertController alertControllerWithTitle:@"专属坐席失败" message:nil preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction * resolvedAction = [UIAlertAction actionWithTitle:@"转其他坐席" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            dispatch_async(dispatch_get_main_queue(), ^{
                __weak QMChatRoomViewController * myChatView = self;
...
/*
 扩展信息字段 customField
 @{@"customField":@{@"扩展信息key":@"扩展信息value",},@"agent":@"0000"}
 专属坐席字段 agent
 **/
- (void)beginNewChat {
    __weak QMChatRoomViewController * myChatView = self;
