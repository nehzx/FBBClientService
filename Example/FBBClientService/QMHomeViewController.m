...
- (void)registerSuccess:(NSNotification *)sender {
    ...
    if ([QMManager defaultManager].selectedPush) {
        [self showChatRoomViewController:@"342d4aa0-d20e-11ea-9819-bf0b406c804b" processType:@"" entranceId:@""]; //
    }else{
...
- (void)buttonAction:(UIButton *)sender {
    ...
    /**
     ...
     userId:    用户ID， 区分用户（只能使用  数字 字母(包括大小写) 下划线 短横线）
                userid是用户的唯一标识，不同的访客不能使用相同的userid
     以上3个都是必填项
     */
    
    [QMConnect registerSDKWithAppKey:@"342d4aa0-d20e-11ea-9819-bf0b406c804b" userName:@"10086" userId:@"10086"];
}
